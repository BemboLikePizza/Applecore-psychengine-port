...
class PlayState extends MusicBeatState
{
	...
	public var opponentStrums2:FlxTypedGroup<Strum>;
	...
	var funnyFloatyBoys:Array<String> = ['bandu', 'unfair-junker', 'bambi-piss-3d'];
	...
	public static var toMake3DNote:Array<String> = ['bandu'];
	...
	override public function create()
	{
		...
		if (SONG == null)
			SONG = Song.loadFromJson('applecore');
		...
		switch (curStage)
		{
			...
			case 'applecore':
				bg1 = new FlxSprite(-600, -200).loadGraphic(Paths.image('hi'));
				bg1.active = true;
				bg1.scale.set(1.75, 1.75);
				add(bg1);

				bg2 = new FlxSprite(-600, -200).loadGraphic(Paths.image('poop'));
				bg2.active = true;
				bg2.scale.set(1.75, 1.75);
				bg2.visible = false;
				add(bg2);

				bg3 = new FlxSprite(-350, -375).loadGraphic(Paths.image('yeah'));
				bg3.active = true;
				bg3.visible = false;
				bg3.scale.set(2, 2);
				add(bg3);
				...
				bg2.shader = testshader2.shader;
				bg3.shader = testshader3.shader;
				...
				for (i in 0...2)
				{
					...
					pizza.frames = Paths.getSparrowAtlas('characters/pizza');
					...
					var spr:FlxSprite = new FlxSprite(FlxG.random.int(100, 1000), FlxG.random.int(100, 500));
					spr.frames = Paths.getSparrowAtlas('characters/minion');
				...
				add(swagger);
				altSong = Song.loadFromJson('altnotes', 'applecore');

				/* don't uncomment this or the game crashes
					what.members[3].visible = false;
					what.members[4].visible = false;
				 */
		...
		if (toMake3DNote.contains(dad.curCharacter.toLowerCase()))
		{
			trace('3D Character Detected! Making Notes To 3D');
			// SONG.arrowSkin = '3DNOTE_assets';
			trace('DUMBSHIT');
			trace('DUMBSHIT');
			trace('DUMBSHIT');
			trace('DUMBSHIT');
			trace('DUMBSHIT');
			// trace('Cur NoteSkin : ' + SONG.arrowSkin);
		}
		...
		opponentStrums2 = new FlxTypedGroup<Strum>();
	...
	public function startCountdown():Void
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			{
				// i lov lua but now is haxe
				// haxe is simple and has simple build
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if (tmr.loopsLeft % 2 == 0)
				{
					...
					if (dad.animation.curAnim != null && !dad.animation.curAnim.name.startsWith('sing') && !dad.stunned && !raged)
					{
						dad.dance();
					}
				}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (altSong != null)
		{
			...
			for (section in altSong.notes)
			{
				...
				{
					var swagNote:Note = new Note(noteJunk[0], Std.int(noteJunk[1] % 4), null, false, false);
					swagNote.isAlt = true;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			// FlxG.log.add(i);
			/*
			var babyArrow:Strum = new Strum(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player);
		 */
			var babyArrow:Strum = new Strum(0, strumLine.y);

			if (Note.CharactersWith3D.contains(dad.curCharacter)
				&& player == 0
				|| Note.CharactersWith3D.contains(boyfriend.curCharacter)
				&& player == 1)
			{
				babyArrow.frames = Paths.getSparrowAtlas('3DNOTE_assets');
	...
	private var downscrollSave:Bool = ClientPrefs.downScroll;
	private var ghosttappingSave:Bool = ClientPrefs.ghostTapping;
	...
	override public function update(elapsed:Float)
	{
		...
		if (toMake3DNote.contains(dad.curCharacter.toLowerCase()))
		{
			if (dad.animation.curAnim.name != null && !dad.animation.curAnim.name.startsWith("sing") && !dad.stunned)
			{
				if (!raged)
				{
					...
					dadmirror.animation.play('idle');
				}
				else
				{
					// lol
					dad.animation.play('fall', false, false, 17);
					dadmirror.animation.play('fall', false, false, 17);
				}
			}
		}
		...
		switch (SONG.song.toLowerCase())
		{
			case 'applecore':
				...
				/*
				var toMove:Float = 0.14;
				var toYMove:Float = 0.2;
				if (curStep >= 897 && curStep < 2543)
				{
					playerStrums.members[0].x += (Math.sin(elapsedtime) * toMove);
					playerStrums.members[1].x -= (Math.sin(elapsedtime) * toMove);

					playerStrums.members[2].x += (Math.sin(elapsedtime) * toMove);
					playerStrums.members[3].x -= (Math.sin(elapsedtime) * toMove);

					playerStrums.members[0].y += (Math.sin(elapsedtime) * toYMove);
					playerStrums.members[1].y -= (Math.sin(elapsedtime) * toYMove);

					playerStrums.members[2].y += (Math.sin(elapsedtime) * toYMove);
					playerStrums.members[3].y -= (Math.sin(elapsedtime) * toYMove);

					for (i in 0...playerStrums.length)
					{
						playerStrums.members[i].setGraphicSize(Std.int(Math.sin(elapsedtime) * 0.2), Std.int(Math.sin(elapsedtime) * 0.4));
						playerStrums.members[i].updateHitbox();
					}
				}
			 */
		}
		...
		{
			if (dad.curCharacter.toLowerCase() == 'bandu')
			{
		...
		if (FlxG.keys.justPressed.ENTER && startedCountdown && canPause)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				...
				else
				{
					...
					ClientPrefs.downScroll = downscrollSave;
					ClientPrefs.ghostTapping = ghosttappingSave;
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (daNote.copyY)
				{
					if (ClientPrefs.downScroll)
					{
						if (SONG.song.toLowerCase() == 'applecore' && unfairPart)
							daNote.y = (strumY
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					else if (!daNote.noAnimation)
					{
						...
						littleIdiot.playAnim(animToPlay + altAnim, true);
		...
		if (isDad)
		{
			if (SONG.song.toLowerCase() == 'applecore')
			{
				if (raged)
				{
					...
					camFollow.x += swagger.cameraPosition[0];
					camFollow.y += swagger.cameraPosition[1];
				}
				else if (!raged && unfairPart)
				{
					defaultCamZoom = 0.4;
					camFollow.set(littleIdiot.getMidpoint().x + 150, littleIdiot.getMidpoint().y - 100);
					camFollow.x += littleIdiot.cameraPosition[0];
					camFollow.y += littleIdiot.cameraPosition[1];
				}
			}
			else
			{
	...
	{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		// Dupe note remove
		var songLOL:String = SONG.song.toLowerCase();
		switch (songLOL)
		{
			case '노래 이름':
				// 특별한 애니메이션 재생하고 죽으려면 이걸 쓰세요
				notes.clear();
				camHUD.visible = false;
				defaultCamZoom = 1;
				camFollowPos.setPosition(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
				dad.playAnim('애니메이션 이름');
				FlxG.sound.play(Paths.sound('소리 이름'));
			...
			case 'applecore':
				switch (curBeat)
				{
					...
					case 224:
						wtfThing = true;
						raged = true;
						what.members[0].alpha = 0;
						what.members[1].alpha = 1;
						what.members[2].alpha = 0;
						what.members[3].alpha = 1;
						what.forEach(function(spr:FlxSprite)
						{
							/*
					...
					case 636:
						unfairPart = true;
						FlxTween.tween(botplayTxt, {y: (FlxG.height / 2) - (botplayTxt.height / 2)}, Conductor.bpm / 1000, {ease: FlxEase.quadInOut});
						...
						gfSpeed = 1;
						wtfThing = false;
						var dumbStupid = new FlxSprite().loadGraphic(Paths.image('characters/poop'));
					...
					case 667:
						FlxTween.tween(littleIdiot, {"scale.x": littleIdiot.scale.x + 2.1, "scale.y": littleIdiot.scale.y + 2.1}, 1.35, {
							...
							onComplete: function(twn:FlxTween)
							{
								iconP2.changeIcon(littleIdiot.healthIcon);
								// it dosent work btw
								// reloadHealthBarColors();
								healthBar.createFilledBar(0xFFB10707, 0xFF31B0D1);
								healthBar.updateBar();
								orbit = false;
								dad.visible = dadmirror.visible = swagger.visible = false;
								var derez = new FlxSprite(dad.getMidpoint().x, dad.getMidpoint().y).loadGraphic(Paths.image('characters/monkey_guy'));
								derez.setPosition(derez.x - derez.width / 2, derez.y - derez.height / 2);
								derez.antialiasing = false;
								add(derez);
								var deez = new FlxSprite(swagger.getMidpoint().x, swagger.getMidpoint().y).loadGraphic(Paths.image('characters/monkey_person'));
								...
								{
									if (littleIdiot.animation.curAnim.finished)
										littleIdiot.dance();
									swagsnd.stop();
					...
					case 1343:
						// Resets original settings
						// or people will uncomfortable for this
						ClientPrefs.ghostTapping = ghosttappingSave;
						ClientPrefs.downScroll = downscrollSave;
				}
		}
	 ...
	 */
		...
		if (curBeat % 2 == 0)
		{
			...
			if (dad.animation.curAnim.name != null && !dad.animation.curAnim.name.startsWith("sing") && !dad.stunned)
			{
				if (!raged && !unfairPart && dad.curCharacter != 'bandu')
				{
					dad.dance();
				}
				if (littleIdiot.animation.curAnim.name != 'inhale')
					littleIdiot.dance();
				swagger.dance();
			}
		}
